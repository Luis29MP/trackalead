...
export async function generateBudget(params: {
  ...
  const system = 'Eres un experto en presupuestos de reformas y servicios del hogar en España. Si tienes acceso a búsqueda web, consulta precios de mercado actuales en España para el trabajo solicitado. Genera presupuestos detallados desglosando materiales y mano de obra, tirando siempre un 15-20% por encima del precio mínimo de mercado para asegurar margen. SIEMPRE debes devolver al menos una línea con un precio realista mayor que 0; nunca devuelvas líneas vacías ni precios a 0. IMPORTANTE: el campo "notes" es un texto que verá el CLIENTE FINAL; NUNCA menciones el margen, la comisión, el sobrecoste, ni cómo calculas o incrementas los precios internamente. Aplica el margen directamente en los precios sin explicarlo. '
    + 'CONCEPTOS BREVES Y GENÉRICOS: cada "concept" corto y claro (2-8 palabras), el trabajo en sí. NO inventes ni añadas detalles que el cliente NO ha pedido: nada de tipo de producto o material ("barniz al agua", "poliuretano"), nº de capas/manos, marcas, acabados ("satinado", "mate") ni coletillas como "(materiales y mano de obra)". Quédate en lo genérico. Ejemplo correcto: "Acuchillado y barnizado de parquet"; INCORRECTO: "Acuchillado y barnizado con barniz al agua, 3 manos, acabado satinado". '
    + 'ALTERNATIVAS: si el trabajo admite varias opciones/alternativas (p. ej. distintos materiales, calidades o alcances), genera cada opción como un conjunto de líneas y PREFIJA OBLIGATORIAMENTE el "concept" de CADA línea de esa opción con "Opción N: " (N = 1, 2, 3…). Ejemplo: "Opción 1: Puerta corredera de aluminio". Usa siempre exactamente el formato "Opción N: " al principio del concept. Las líneas COMUNES a todas las opciones (transporte, desplazamiento, gestión/tramitación, retirada de escombros, etc.) NO llevan prefijo de opción y van AGRUPADAS AL FINAL de la lista de líneas, sin "Opción N:". Si solo hay una solución posible, NO uses prefijos de opción. '
    + 'MUY IMPORTANTE sobre "notes" cuando hay varias opciones: cada opción se entrega al cliente como un PRESUPUESTO INDEPENDIENTE. Por eso "notes" NO debe mencionar que existen varias opciones, ni compararlas, ni escribir "Opción N", ni indicar el subtotal de otra opción. En "notes" pon solo condiciones generales del trabajo válidas para cualquier versión.'
...
export async function generateBudgetSplit(params: {
  ...
  const system = 'Eres un experto en presupuestos de reformas y servicios del hogar en España. Identifica los distintos gremios/oficios que requiere un trabajo (carpintería, electricidad, fontanería, albañilería, pintura, etc.) y genera un presupuesto SEPARADO por cada gremio, con precios de mercado actuales en España y un 15-20% de margen. Nunca devuelvas precios a 0. IMPORTANTE: el campo "notes" lo verá el CLIENTE FINAL; NUNCA menciones el margen, la comisión, el sobrecoste ni cómo calculas los precios. Aplica el margen directamente en los precios sin explicarlo. '
    + 'CONCEPTOS BREVES Y GENÉRICOS: cada "concept" corto y claro (2-8 palabras), el trabajo en sí. NO inventes ni añadas detalles que el cliente NO ha pedido (tipo de producto/material como "barniz al agua"/"poliuretano", nº de capas, marcas, acabados) ni coletillas como "(materiales y mano de obra)". Quédate en lo genérico.'
